  ...
  void initializeFirebase() async {
    ...
    if (settings.authorizationStatus == AuthorizationStatus.authorized) {
      ...
      FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
        ...
        // Aquí obtienes los datos enviados en la notificación
        if (message.data.containsKey('screen')) {
          String screen = message.data['screen'];
          String userId = message.data['userId'];

          // Aquí navegas a la pantalla según el dato recibido
          if (screen == 'profile') {
            Navigator.push(
              context,
              MaterialPageRoute(
...
class _MisMascotasPageState extends State<MisMascotasPageInicio> {
  ...
  Future<List<Activity>> fetchEventsForMascota(
    ...
    try {
      final url =
          Uri.parse('$baseUrl/api/activity/activity-mascota?id=$mascotaId&status=Aprobada');
